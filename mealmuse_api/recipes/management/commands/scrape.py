...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--urls", action="store_true", help="Scrapes all recipe urls"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if not any(
            [options["urls"], options["recipes"], options["specific_recipe_slug"]]
        ):
    ...
    def scrape_specific_recipe(self, slug: str):
        recipe, image_url = self._scrape_recipe_url(
            "{base_url}/recipes/{slug}".format(base_url=URL_NYT, slug=slug.strip())
        )
        # save search vector
    ...
    def _scrape_urls(self):
        ...
        while True:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        "Bad sequential response #{} for {}".format(
                            sequential_failures, url
                        )
                    )
                )
                ...
                if sequential_failures > 5:
                    self.stdout.write(
                        self.style.WARNING(
                            "Too many failures for {}, continuing".format(url)
                        )
                    )
                    page += 1
                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    "Fetched {} with {} recipes".format(url, len(recipes))
                )
            )
            if recipes:
                for recipe in recipes:
                    ...
                    if not recipe_url:
                        logging.warning(
                            f'${url} is missing "url" in an article, skipping'
                        )
                ...
                self.stdout.write(
                    self.style.SUCCESS("No recipes found on URl {}".format(url))
                )
            ...
            if page_recipe_urls.issubset(recipe_urls):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"identical page response #{identical_page_failures} for {url}"
                    )
                )
                if identical_page_failures >= 20:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Pages have been identical the last {identical_page_failures} times so stopping scrape on page {page}"
                        )
                    )
                    break
            else:
        ...
        json.dump(
            {"urls": list(recipe_urls)},
            open(os.path.join(CACHE_DIR, "urls.json"), "w"),
            indent=2,
        )
        self.stdout.write(
            self.style.SUCCESS("Completed {} urls".format(len(recipe_urls)))
        )
    ...
    def _scrape_recipes(self):
        ...
        if not os.path.exists(urls_file):
            self.stdout.write(
                self.style.ERROR(
                    f'"{urls_file}" does not exist.  Run with --urls first'
                )
            )
            sys.exit(1)
        ...
        urls_existing = [
            f"/recipes/{slug}"
            for slug in list(Recipe.objects.values_list("slug", flat=True))
        ]
        ...
        for i, url in enumerate(urls_all):
            ...
            try:
                recipe, image_url = self._scrape_recipe_url(
                    "{base_url}{url}".format(base_url=URL_NYT, url=url)
                )
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        "Could not download image {} for {}".format(image_url, recipe)
                    )
                )

            if i != 0 and i % 100 == 0:
                self.stdout.write(
                    self.style.SUCCESS("Scraped {} recipes so far".format(i))
                )

            recipes_scraped += 1

        self.stdout.write(
            self.style.SUCCESS("Scraped {} recipes total".format(recipes_scraped))
        )
    ...
    def _scrape_recipe_url(self, url: str) -> Tuple[Recipe, str]:
        ...
        if not recipe_data.get("ingredients"):
            raise Exception(
                f"skipping recipe {url} without ingredients or instructions"
            )
    ...
    def _replace_recipe_links_to_internal(
        ...
        re_search = r"https?://{base_url}/recipes/".format(
            base_url=domain_parsed.hostname
        )
        re_replace = "/#/recipe/"
